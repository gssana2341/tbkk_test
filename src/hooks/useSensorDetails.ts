...
export function useSensorDetails({ sensorId }: UseSensorDetailsProps) {
    ...
    const sortedDatetimes = useMemo(
        () => [...datetimes].sort((a, b) => new Date(a).getTime() - new Date(b).getTime()),
        [datetimes]
    );
    ...
    const fetchSensorDetails = useCallback(async (id: string) => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/sensors/${id}`, {
                cache: "no-store",
    ...
    const fetchSensorLastData = useCallback(async (id: string, datetime?: string) => {
        try {
            const url = datetime
            ...
            if (response.ok) {
                ...
                const transformedData: SensorLastData = {
                    ...
                    data: {
                        ...
                        level_temperature: data.level_temperature || data.data?.level_temperature,
            ...
            const withDataResponse = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/sensors/with-last-data`, {
                cache: "no-store",
                headers: {
                    "Cache-Control": "no-cache",
                    ...(tokenFallback ? { Authorization: `Bearer ${tokenFallback}` } : {}),
                },
            });

            if (withDataResponse.ok) {
                const allSensors = (await withDataResponse.json()) as WithLastDataSensor[];
                const sensorData = allSensors.find((s) => s.id === id);
                if (sensorData) {
                    const transformedData: SensorLastData = {
                        id: sensorData.id,
                        name: sensorData.name || sensorData.sensor_name || sensorData.id || "Unknown Sensor",
    ...
    const fetchSensorConfig = useCallback(async (id: string) => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/sensors/${id}/config`, {
                cache: "no-store",
                headers: { "Cache-Control": "no-cache" },
            });
            ...
            setConfigData((prev) => ({
                ...
                thresholdMedium: data.threshold_medium?.toString() || prev.thresholdMedium,
    ...
    const fetchSensorData = useCallback(async () => {
        ...
        try {
            ...
            if (histResponse.ok) {
                ...
                const items = Array.isArray(histData) ? histData : (histData.history || histData.data || []);
    ...
    }, [sensorId, fetchSensorLastData, fetchSensorConfig, fetchSensorDetails, fetchSensorHistory]);
    ...
    useEffect(() => {
        ...
        const fetchImage = async () => {
            try {
                const token = localStorage.getItem("auth_token");
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/sensors/${sensorId}/image`, {
                    headers: token ? { Authorization: `Bearer ${token}` } : {},
                });
                if (response.ok) {
    ...
    const allChartData = useMemo(() => {
        ...
        const hasLast32Data = Array.isArray(data.last_32_h) && data.last_32_h.length > 0;
        ...
        let accHData = hasAccData ? (data.acc_h || []) : (hasLast32Data ? (data.last_32_h?.flat() || []) : (data.a_h_data || []));
        let accVData = hasAccData ? (data.acc_v || []) : (hasLast32Data ? (data.last_32_v?.flat() || []) : (data.a_v_data || []));
        let accAData = hasAccData ? (data.acc_a || []) : (hasLast32Data ? (data.last_32_a?.flat() || []) : (data.a_a_data || []));
        ...
        let freqHData = hasAccData ? (data.freq_h || []) : (hasAHData ? (data.f_point_h || []) : []);
        let freqVData = hasAccData ? (data.freq_v || []) : (Array.isArray(data.a_v_data) && data.a_v_data.length > 0 ? (data.f_point_v || []) : []);
        let freqAData = hasAccData ? (data.freq_a || []) : (Array.isArray(data.a_a_data) && data.a_a_data.length > 0 ? (data.f_point_a || []) : []);

        // Final fallback if still empty but we have some magnitude data
        if (freqHData.length === 0 && (hasAccData || hasLast32Data || hasAHData)) {
            const magLen = hasAccData ? data.acc_h?.length : (hasLast32Data ? data.last_32_h?.flat().length : data.a_h_data?.length);
            freqHData = Array.from({ length: magLen || 0 }, (_, i) => i + 1);
        }
        if (freqVData.length === 0 && (hasAccData || hasLast32Data || (Array.isArray(data.a_v_data) && data.a_v_data.length > 0))) {
            const magLen = hasAccData ? data.acc_v?.length : (hasLast32Data ? data.last_32_v?.flat().length : data.a_v_data?.length);
            freqVData = Array.from({ length: magLen || 0 }, (_, i) => i + 1);
        }
        if (freqAData.length === 0 && (hasAccData || hasLast32Data || (Array.isArray(data.a_a_data) && data.a_a_data.length > 0))) {
            const magLen = hasAccData ? data.acc_a?.length : (hasLast32Data ? data.last_32_a?.flat().length : data.a_a_data?.length);
            freqAData = Array.from({ length: magLen || 0 }, (_, i) => i + 1);
        }

        const totalTime = configData.lor / configData.fmax;
        const timeInterval = accHData.length > 1 ? totalTime / (accHData.length - 1) : 0;

        const processAxis = (accG: number[], accMm: number[], velMm: number[], freqs: number[]) => {
            ...
            if (finalVelMm.length === 0 && finalAccMm.length > 0 && freqs.length === finalAccMm.length) {
                finalVelMm = finalAccMm.map((a, i) => calculateVelocityFromFrequency(a, freqs[i]));
            }
            return { accMm: finalAccMm, velMm: finalVelMm };
        };

        const hResult = processAxis(accHData, data.a_h_data || [], data.v_h_data || [], freqHData);
        const vResult = processAxis(accVData, data.a_v_data || [], data.v_v_data || [], freqVData);
        const aResult = processAxis(accAData, data.a_a_data || [], data.v_a_data || [], freqAData);

        const axes = {
            h: { acc_g: accHData, acc_mms2: hResult.accMm, vel_mms: hResult.velMm, freq: freqHData, fPoints: data.f_point_h, g_rms: data.g_rms_h, a_rms: data.a_rms_h, velo_rms: data.velo_rms_h },
            v: { acc_g: accVData, acc_mms2: vResult.accMm, vel_mms: vResult.velMm, freq: freqVData, fPoints: data.f_point_v, g_rms: data.g_rms_v, a_rms: data.a_rms_v, velo_rms: data.velo_rms_v },
            a: { acc_g: accAData, acc_mms2: aResult.accMm, vel_mms: aResult.velMm, freq: freqAData, fPoints: data.f_point_a, g_rms: data.g_rms_a, a_rms: data.a_rms_a, velo_rms: data.velo_rms_a },
        };
        ...
        Object.entries(axes).forEach(([axisKey, axisData]) => {
            units.forEach((unit) => {
                ...
                if (unit === "Acceleration (G)") { unitMag = axisData.acc_g; rmsOverride = axisData.g_rms; }
                else if (unit === "Acceleration (mm/s²)") { unitMag = axisData.acc_mms2; rmsOverride = axisData.a_rms; }
                else if (unit === "Velocity (mm/s)") { unitMag = axisData.vel_mms; rmsOverride = axisData.velo_rms; }

                const freqSrc = axisData.fPoints && axisData.fPoints.length > 0 ? axisData.fPoints : axisData.freq;
                ...
                const isSpectrumData = unitMag.length > 0 && unitMag.length <= (chartConfig.lor * 0.5);

                let accInput = isSpectrumData ? [] : unitMag;
                let freqInput = isSpectrumData ? unitMag : [];

                result[axisKey][unit] = prepareChartData(accInput, freqInput, unit, timeInterval, chartConfig, freqSrc, rmsOverride);
            });
        });
    ...
    const xStats = useMemo(() => {
        if (loading || !sensorLastData?.data) return { accelTopPeak: "0.000", velocityTopPeak: "0.000", dominantFreq: "0.000", topPeaks: { G: [], mmPerS2: [], mmPerS: [] } };
