...
export function useRegisterSensorForm() {
    ...
    const form = useForm<FormValues>({
        ...
        defaultValues: {
            sensors: [
                { ...DEFAULT_SENSOR_VALUES, sensorType: "Master", notes: "Master Sensor" },
                { ...DEFAULT_SENSOR_VALUES, sensorType: "Satellite", notes: "Satellite 1" },
                { ...DEFAULT_SENSOR_VALUES, sensorType: "Satellite", notes: "Satellite 2" },
                { ...DEFAULT_SENSOR_VALUES, sensorType: "Satellite", notes: "Satellite 3" },
            ],
        },
    });
    ...
    const handleRealSubmit = async (values: FormValues) => {
            ...
            try {
                const updates = values.sensors
                    ...
                    .map(async (sensorData, index) => {
                        ...
                        if (!targetId)
                            return { success: false, name: sensorData.serialNumber, error: "Missing ID" };
                        ...
                        if (selectedImages[index]) {
                            try {
                                ...
                                console.error(`Failed to upload image for sensor ${targetId}`, imgErr);
                            }
                        }
        ...
        try {
            const validSensors = values.sensors
                ...
                .filter((sensor) => sensor.serialNumber && sensor.serialNumber.length > 0);
            if (validSensors.length === 0) {
                toast({
                    title: "No Sensors to Register",
                    description: "Please fill in at least one sensor (Master is recommended).",
                    variant: "destructive",
                });
                return;
            }
            ...
            const payload = validSensors.map((sensorData) => {
                ...
                return {
                    ...
                    temperature_threshold_max: sensorData.temperatureThresholdMax
                        ...
                        : null,
                    alarm_ths: sensorData.alarmThreshold ? Number(sensorData.alarmThreshold) : null,
                };
            });
