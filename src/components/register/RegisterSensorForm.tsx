  ...
  .superRefine((data, ctx) => {
    ...
    if (data.serialNumber && data.serialNumber.length > 0) {
      if (data.serialNumber.length !== 12) {
        ...
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "Serial number must contain only hexadecimal characters (0-9, A-F)",
          path: ["serialNumber"],
        });
      }
...
export default function RegisterSensorForm() {
  ...
  const [machineNameSuggestions, setMachineNameSuggestions] = useState<string[]>([]);
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      sensors: [
        { ...defaultSensorValues, sensorType: "Master", notes: "Master Sensor" },
        { ...defaultSensorValues, sensorType: "Satellite", notes: "Satellite 1" },
        { ...defaultSensorValues, sensorType: "Satellite", notes: "Satellite 2" },
        { ...defaultSensorValues, sensorType: "Satellite", notes: "Satellite 3" },
      ],
    },
  });
      ...
      try {
        ...
        if (response.ok) {
          ...
          const mappedSensor = {
            ...
            motorStartTime: data.motor_start_time ? parseCustomDate(data.motor_start_time) : new Date(),
            ...
            temperatureThresholdMin: data.temperature_threshold_min?.toString() || "",
            timeInterval: data.time_interval?.toString() || "120",
            lor: data.lor?.toString() || "5600",
            frequencyMax: data.fmax?.toString() || "9000",
            temperatureThresholdMax: data.temperature_threshold_max?.toString() || "",
          ...
          if (!data.sensor_type || data.sensor_type === "Master") {
            try {
              const allSensorsResponse = await fetch(`${"/api"}/sensors/with-last-data`, {
                headers: { Authorization: `Bearer ${localStorage.getItem("auth_token") || ""}` },
              });
              if (allSensorsResponse.ok) {
                const allSensors = await allSensorsResponse.json();
                const satellites = allSensors.filter((s: any) => s.machine_no === data.machine_no && s.sensor_type === "Satellite" && s.id !== data.id);
                satellites.sort((a: any, b: any) => (a.sensor_name || "").localeCompare(b.sensor_name || ""));
                satellites.forEach((sat: any, index: number) => {
                  if (index < 3) {
                    newSensors[index + 1] = {
                      ...
                      motorStartTime: sat.motor_start_time ? parseCustomDate(sat.motor_start_time) : new Date(),
                      ...
                      temperatureThresholdMin: sat.temperature_threshold_min?.toString() || "",
                      timeInterval: sat.time_interval?.toString() || "120",
                      lor: sat.lor?.toString() || "5600",
                      frequencyMax: sat.fmax?.toString() || "9000",
                      temperatureThresholdMax: sat.temperature_threshold_max?.toString() || "",
          ...
          toast({ title: "Edit Mode", description: "Loaded sensor data for editing." });
        }
  ...
  const handleCropComplete = (croppedBlob: Blob) => {
    if (cropIndex !== null) {
      const file = new File([croppedBlob], "sensor-image.jpg", { type: "image/jpeg" });
      setSelectedImages((prev) => ({ ...prev, [cropIndex]: file }));
      setImagePreviews((prev) => ({ ...prev, [cropIndex]: URL.createObjectURL(croppedBlob) }));
    }
      ...
      try {
        const updates = values.sensors
          ...
          .map(async (sensorData, index) => {
            ...
            if (!targetId) return { success: false, name: sensorData.serialNumber, error: "Missing ID" };
            ...
            const payload = {
              ...
              temperature_threshold_min: parseFloat(sensorData.temperatureThresholdMin || "0"),
              temperature_threshold_max: parseFloat(sensorData.temperatureThresholdMax || "0"),
            ...
            const response = await fetch(`${"/api"}/sensors/${targetId}/config`, {
              method: "PUT",
            ...
            if (selectedImages[index]) {
              try { await uploadSensorImage(targetId, selectedImages[index]); }
              catch (imgErr) { console.error(`Failed to upload image for sensor ${targetId}`, imgErr); }
            }
        ...
        if (failures.length > 0) {
          toast({ title: "Update Completed with Errors", description: `Updated ${results.length - failures.length} sensors. Failed: ${failures.length}`, variant: "destructive" });
        } else {
          toast({ title: "Update Successful", description: `All ${results.length} sensors updated successfully.` });
          router.push(`/sensors/${editId}`);
        }
      } catch (error) {
        console.error("Error updating sensor:", error);
        toast({ title: "Update Failed", description: "Failed to update sensors.", variant: "destructive" });
      } finally { setIsSubmitting(false); }
      return;
    }

    try {
      const validSensors = values.sensors.map((sensor, index) => ({ ...sensor, originalIndex: index })).filter((sensor) => sensor.serialNumber && sensor.serialNumber.length > 0);
      if (validSensors.length === 0) {
        toast({ title: "No Sensors to Register", description: "Please fill in at least one sensor (Master is recommended).", variant: "destructive" });
        return;
      }
      ...
      const payload = validSensors.map((sensorData) => {
        ...
        return {
          ...
          max_frequency: sensorData.frequencyMax ? Number(sensorData.frequencyMax) : null,
          note: sensorData.notes || "",
          threshold_min: sensorData.warningThreshold ? Number(sensorData.warningThreshold) : null,
          threshold_medium: sensorData.concernThreshold ? Number(sensorData.concernThreshold) : null,
          threshold_max: sensorData.damageThreshold ? Number(sensorData.damageThreshold) : null,
          temperature_threshold_min: sensorData.temperatureThresholdMin ? Number(sensorData.temperatureThresholdMin) : null,
          temperature_threshold_max: sensorData.temperatureThresholdMax ? Number(sensorData.temperatureThresholdMax) : null,
        };
      });
      ...
      for (const sensorData of validSensors) {
        ...
        if (imageFile) {
          ...
          const createdSensor = createdSensors.find((s: any) => s.mac_address === mac || s.sensor?.mac_address === mac);
          const sensorId = createdSensor?.id || createdSensor?.sensor?.id;
          if (sensorId) { try { await uploadSensorImage(sensorId, imageFile); } catch (e) { console.error("Image upload failed", e); } }
        }
      }
      toast({ title: "Registration Complete", description: `Successfully registered ${validSensors.length} sensors.` });
      router.push("/");
    } catch (error) {
          ...
          <CardHeader className="p-0 border-b border-gray-700">
            {(!editId || form.watch("sensors.0.sensorType") === "Master") && (
              <TabsList className="flex w-full justify-start bg-transparent p-0">
                ...
                <TabsTrigger value="sat1" className="rounded-none border-b-2 border-transparent px-6 py-3 data-[state=active]:border-blue-600 data-[state=active]:text-blue-600 data-[state=active]:bg-transparent data-[state=active]:shadow-none">Satellite 1</TabsTrigger>
                <TabsTrigger value="sat2" className="rounded-none border-b-2 border-transparent px-6 py-3 data-[state=active]:border-blue-600 data-[state=active]:text-blue-600 data-[state=active]:bg-transparent data-[state=active]:shadow-none">Satellite 2</TabsTrigger>
                <TabsTrigger value="sat3" className="rounded-none border-b-2 border-transparent px-6 py-3 data-[state=active]:border-blue-600 data-[state=active]:text-blue-600 data-[state=active]:bg-transparent data-[state=active]:shadow-none">Satellite 3</TabsTrigger>
              </TabsList>
            )}
          </CardHeader>
          <CardContent className="pt-6">
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit, (errors) => {
                ...
                toast({ title: "Validation Error", description: "Please check the form for missing or invalid fields.", variant: "destructive" });
              })} className="space-y-6">
                <TabsContent value="master" className="mt-0">
                  <SensorFormContent form={form} index={0} areaSuggestions={areaSuggestions} machineNameSuggestions={machineNameSuggestions} imagePreview={imagePreviews[0]} onImageChange={(e) => handleImageChange(0, e)} />
                </TabsContent>
                <TabsContent value="sat1" className="mt-0">
                  <SensorFormContent form={form} index={1} areaSuggestions={areaSuggestions} machineNameSuggestions={machineNameSuggestions} imagePreview={imagePreviews[1]} onImageChange={(e) => handleImageChange(1, e)} />
                </TabsContent>
                <TabsContent value="sat2" className="mt-0">
                  <SensorFormContent form={form} index={2} areaSuggestions={areaSuggestions} machineNameSuggestions={machineNameSuggestions} imagePreview={imagePreviews[2]} onImageChange={(e) => handleImageChange(2, e)} />
                </TabsContent>
                <TabsContent value="sat3" className="mt-0">
                  <SensorFormContent form={form} index={3} areaSuggestions={areaSuggestions} machineNameSuggestions={machineNameSuggestions} imagePreview={imagePreviews[3]} onImageChange={(e) => handleImageChange(3, e)} />
                </TabsContent>
                ...
                <div className="grid grid-cols-3 items-center pt-4 w-full">
                  <div className="justify-self-start">
                    {!editId && (
                      <Button type="button" onClick={() => {
                        ...
                        setImagePreviews({});
                        setSelectedImages({});
                        toast({ title: "Form Reset", description: "All fields have been reset to default values." });
                      }} className="bg-[#E35D5D] text-white hover:bg-red-600 w-24">Reset</Button>
                    )}
                  </div>
                  <div className="justify-self-center flex gap-4">
                    <Button type="button" variant="outline" onClick={() => {
                      if (editId) window.history.back();
                      else activeTab === "master" ? router.push("/") : handleBack();
                    }} disabled={isSubmitting} className="bg-[#FFFEFF] text-black hover:bg-gray-100 w-24">{editId || activeTab === "master" ? "Cancel" : "Back"}</Button>
                    <Button type="submit" disabled={isSubmitting} className="bg-[#2186F3] text-white hover:bg-blue-600 w-24">
                  ...
                  <div className="justify-self-end">
                    {!editId && activeTab !== "sat3" && (
                      <Button type="button" onClick={handleNext} disabled={isSubmitting} className="bg-[#2186F3] text-white hover:bg-blue-600 w-24">Next</Button>
                    )}
                  </div>
                </div>
              </form>
