...
export const VibrationAnalysisSection: React.FC<VibrationAnalysisSectionProps> = ({
    configData,
    selectedAxis,
    setSelectedAxis,
    selectedUnit,
                            ...
                            <div className="mb-4">
                                ...
                                <div className="space-y-4">
                                    <div className="grid grid-cols-2 gap-6 pb-3 border-b-2 border-[#374151]">
                                        <div className="text-base font-bold text-white">RMS :</div>
                                        <div className="text-base font-bold text-white text-right">Frequency</div>
                                    </div>
                                    {hasData && vibrationData.topPeaks ? (
                                        vibrationData.topPeaks.map((row: any, i: number) => (
                                            <div key={i} className="grid grid-cols-2 gap-6">
                                                <div className="text-lg font-medium text-white">{row.rms}</div>
                                                <div className="text-lg font-medium text-white text-right">{row.frequency} Hz</div>
                                            </div>
                                        ))
                                    ) : (
                                        <div className="py-8 text-center text-gray-300 text-base">No data available</div>
                                    )}
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="overflow-x-auto max-h-80 overflow-y-auto custom-scrollbar">
                                <table className="min-w-full text-base">
                                    <thead className="sticky top-0 bg-[#0B1121]">
                                        <tr>
                                            <th className="text-left px-3 py-3 font-bold text-white">Date & Time</th>
                                            <th className="text-right px-3 py-3 font-bold text-white">RMS Overall</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {hasData && history.length > 0 ? (
                                            ...
                                            <tr>
                                                <td colSpan={2} className="px-3 py-8 text-center text-gray-500 text-base">
                                        ...
                                        graphic: [
                                            {
                                                ...
                                                bottom: 20,
                                                z: 10,
                                                style: { text: "Time (s)", fill: "#fff", fontWeight: 500, fontSize: 12 },
                                            },
                                        ],
                                        yAxis: {
                                            type: "value",
                                            name: (vibrationData.yAxisLabel || selectedUnit)
                                                .replace("Acceleration ", "").replace("Velocity ", "").replace("(", "").replace(")", ""),
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-gray-100 mb-2">
                                    Frequency Domain : {selectedAxis === "H-axis" ? "H" : selectedAxis === "V-axis" ? "V" : "A"}
                                </h3>
                                <div className="h-[300px] w-full bg-[#030616] border-[1.35px] border-[#374151] rounded-lg p-4 relative">
                                    {vibrationData.hasData && vibrationData.freqData?.labels?.length > 0 ? (
                                        <ReactECharts
                                            option={{
                                                ...
                                                yAxis: {
                                                    type: "value",
                                                    name: selectedUnit === "Acceleration (G)" ? "G" : (selectedUnit.includes("mm/s²") ? "mm/s²" : "mm/s"),
                                                    nameLocation: "end",
                                                    nameTextStyle: { color: "#ffffff", fontSize: 11, align: "right", padding: [0, 10, 0, 0] },
