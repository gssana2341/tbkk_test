...
> = ({
  ...
  history,
    ...
    const freqChartRef = useRef<any>(null);
    const [isHoveringFreq, setIsHoveringFreq] = useState(false);

    const getTop10Peaks = () => {
      if (!hasData || !vibrationData.freqData?.datasets?.[0]?.data) return [];

      const data = vibrationData.freqData.datasets[0].data;
      const labels = vibrationData.freqData.labels;
            ...
            <div>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div
                    className="h-[300px] w-full bg-[#030616] border-[1.35px] border-[#374151] rounded-lg p-4 relative"
                    onMouseEnter={() => setIsHoveringFreq(true)}
                    onMouseLeave={() => setIsHoveringFreq(false)}
                  >
                    {vibrationData.hasData &&
                      vibrationData.freqData?.labels?.length > 0 ? (
                      <ReactECharts
                        onChartReady={(instance) => {
                          freqChartRef.current = instance;
                        }}
                        onEvents={{
                          click: (params: any) => {
                            if (params.componentType === 'markPoint' && freqChartRef.current) {
                              const index = params.data.coordIndex;
                              const chartInstance = freqChartRef.current;

                              // Zoom range: ± 250 data points around peak for consistent detail
