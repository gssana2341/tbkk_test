...
export default function SensorHistoryPage() {
  ...
  useEffect(() => {
    async function fetchHistory() {
      ...
      try {
        ...
        setHistory(mappedHistory.sort((a, b) => new Date(a.datetime).getTime() - new Date(b.datetime).getTime()));
      } catch (err) {
  ...
  const chartOption = useMemo(() => {
    ...
    const getVal = (item: HistoryItem, axis: 'h' | 'v' | 'a') => {
      if (selectedUnit === "Acceleration RMS (G)") return axis === 'h' ? item.g_rms_h : axis === 'v' ? item.g_rms_v : item.g_rms_a;
      if (selectedUnit === "Acceleration(mm/s²)") return axis === 'h' ? item.a_rms_h : axis === 'v' ? item.a_rms_v : item.a_rms_a;
      return axis === 'h' ? item.velo_rms_h : axis === 'v' ? item.velo_rms_v : item.velo_rms_a; // Velocity
    };
    ...
    const markArea = {
      ...
      data: [
        ...
        [{ yAxis: THRESHOLDS.RED_START, itemStyle: { color: "#FF4D4D" } }, { yAxis: 100 }],
      ]
    };
    ...
    return {
      ...
      yAxis: {
        ...
        min: 0,
        max: THRESHOLDS.RED_START + 2, // Ensure red zone is visible
      },
      series: series,
        ...
        <Card className="bg-[#232e3c] border-gray-700">
          <CardContent className="p-4 flex flex-wrap gap-6 items-center">
            ...
            <div className="flex gap-2 ml-auto">
              <Button variant="outline" className="border-gray-600 text-gray-300 hover:bg-gray-700 hover:text-white">Export CSV</Button>
              <Button variant="outline" className="border-gray-600 text-gray-300 hover:bg-gray-700 hover:text-white">Export Excel</Button>
            </div>
          ...
          <CardContent className="p-6">
            ...
            {!loading && history.length > 0 && chartOption && (
              <div className="space-y-4">
                <ReactECharts option={chartOption} style={{ height: '400px', width: '100%' }} theme="dark" notMerge={true} />

                {/* Custom Legend UI */}
                <div className="flex justify-center gap-12 mt-8">
                  <div className="flex flex-col items-center gap-2">
                    <div className="w-48 h-1 bg-[#00E5FF] rounded-full"></div>
                    <span className="text-lg font-bold text-white uppercase tracking-wider">H-axis</span>
                  </div>
                  <div className="flex flex-col items-center gap-2">
                    <div className="w-48 h-1 bg-[#4C6FFF] rounded-full"></div>
                    <span className="text-lg font-bold text-white uppercase tracking-wider">V-axis</span>
                  </div>
                  <div className="flex flex-col items-center gap-2">
                    <div className="w-48 h-1 bg-[#C77DFF] rounded-full"></div>
                    <span className="text-lg font-bold text-white uppercase tracking-wider">A-axis</span>
                  </div>
                </div>
              </div>
            )}
                ...
                <thead>
                  <tr className="border-b border-gray-700 text-gray-400">
                    ...
                    {(selectedAxis === 'all' || selectedAxis === 'h') && (
                      <th className="py-3 font-medium text-[#00E5FF]">H ({selectedUnit})</th>
                    )}
                    {(selectedAxis === 'all' || selectedAxis === 'v') && (
                      <th className="py-3 font-medium text-[#4C6FFF]">V ({selectedUnit})</th>
                    )}
                    {(selectedAxis === 'all' || selectedAxis === 'a') && (
                      <th className="py-3 font-medium text-[#C77DFF]">A ({selectedUnit})</th>
                    )}
                  </tr>
                </thead>
                <tbody className="text-gray-200">
                  {history
                    .sort((a, b) => new Date(b.datetime).getTime() - new Date(a.datetime).getTime()) // Latest to Oldest
                    ...
                    .map((item, i) => {
                      ...
                      return (
                        <tr key={i} className="border-b border-gray-800 hover:bg-[#2b394b]">
                          <td className="py-4 font-mono">{dateStr}</td>
                          <td className="py-4 font-mono">{timeStr}</td>
                          {(selectedAxis === 'all' || selectedAxis === 'h') && <>
                            <td className="py-4 font-mono">{getVal(item, 'h').toFixed(3)}</td>
                          </>}
                          {(selectedAxis === 'all' || selectedAxis === 'v') && <>
                            <td className="py-4 font-mono">{getVal(item, 'v').toFixed(3)}</td>
                          </>}
                          {(selectedAxis === 'all' || selectedAxis === 'a') && <>
                            <td className="py-4 font-mono">{getVal(item, 'a').toFixed(3)}</td>
                          </>}
                        </tr>
                      );
                    })}
